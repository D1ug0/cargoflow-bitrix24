...
function message(
  ...
  type: IntegrationEnvelope['type'] = 'trip.create',
): IntegrationEnvelope {
  return {
...
describe('TripSyncService', () => {
  ...
  it('отправляет назначенный экипаж и стадию после trip.update', async () => {
    process.env.BITRIX_WEBHOOK_URL = 'https://example.bitrix24.ru/rest/1/secret/';
    const locks = {
      withLock: vi.fn(async (_key: string, task: () => Promise<void>) => task()),
    };
    ...
    await service.process(message({ bitrixDealId: 1001, tripId: 'trip-1' }, 'trip.update'));
