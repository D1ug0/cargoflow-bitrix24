...
DTO запрещают неизвестные поля. Клиент может передать `x-correlation-id` в каноническом формате
UUID. API возвращает его в ответе, а при отсутствующем или некорректном значении генерирует новый
UUID. Ошибка содержит `statusCode`, `code`, безопасное `message`, `correlationId`, время и путь.
Внутренние детали 5xx не возвращаются клиенту.
Журнал `GET /integrations` принимает проверенные фильтры `status` и `correlationId`. Поиск по
`correlationId` точный: это позволяет собрать записи одного события из API и worker, не смешивая
их с соседними операциями.
